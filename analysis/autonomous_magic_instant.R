...
path_experiments <- "../results/autonomous-05-06-2021_4/"
...
process <- function(path){
  ...
  bike_stats <- bike_trips %>% 
    ...
    merge(df_config) %>% 
    dplyr::summarise(
      time_pickup = 0, #sum(ifelse(trip_type==1, time_ride, 0)), # [s]
...
parameters <- variables[3:12][-3]
parameters_labels <- c(
  "Mode", "Fleet size [-]", "Average riding speed [km/h]", "Average autonomous driving speed [km/h]",
  "Minimum battery level [%]", "Battery autonomy [km]", "Battery recharge time [h]", "Magic beta [%]", "User trip file [-]")
metrics <- variables[13:length(variables)]
metrics_labels <- c(
...
grid <- c(6,6,6,6,6,5)
effect <- rep(rep(seq(3,8), grid), 10)
...
for(i in parameters_idx){
  ...
  df %>% 
    ...
    ggplot2::ggsave(paste0("figures/plot_AU_IR", "_x_", "num_bikes", "_z_", p, ".pdf"), 
                    width=9, height = 11)
...
write.csv(file="autonomous_instant_metrics_multiple.csv", row.names = F)

df %>% 
  dplyr::filter(num_bikes == 1000) %>% 
  dplyr::slice(1:168) %>% 
  write.csv(file="autonomous_instant_metrics_single.csv", row.names = F)
