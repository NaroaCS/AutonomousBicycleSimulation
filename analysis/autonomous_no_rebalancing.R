...
path_experiments <- "../results/autonomous-06-06-2021_1/"
...
parameters_idx <- setdiff(seq(length(parameters)), c(1,2,9,10))
...
for(i in parameters_idx){
  ...
  df %>% 
    ...
    ggplot2::ggsave(paste0("figures/plot_AU_NR", "_x_", "num_bikes", "_z_", p, ".pdf"), 
                    width=9, height = 11)
}
...
df %>% 
  dplyr::filter(
    ...
    user_trips_file==0,) %>% View
  write.csv(file="autonomous_no_rebalancing_metrics_multiple.csv", row.names = F)

df %>% 
  dplyr::filter(num_bikes == 1000) %>% 
  dplyr::slice(1:167) %>% 
  write.csv(file="autonomous_no_rebalancing_metrics_single.csv", row.names = F)
