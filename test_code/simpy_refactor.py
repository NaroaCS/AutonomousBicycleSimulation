...
data = {
    # Size of the grid
    "grid": {"xlim": [0, 100], "ylim": [0, 100]},
    # Docking stations with their location and capacity
    "stations": [{"location": [5, 5], "capacity": 3}, {"location": [10, 10], "capacity": 3}, {"location": [90, 90], "capacity": 2}, {"location": [95, 95], "capacity": 2},],
...
class Station:
    ...
    def print_info(self):  # Prints time, station, and nbikes/capacity
        print("[%.2f] Station %d has %d bikes out of %d" % (self.env.now, self.station_id, self.container.level, self.capacity))
...
class StationBike(Bike):
    ...
    def register_pull(self, agent_id):
        if self.print:
            print("[%.2f] Bike %d pulled from station %d by agent %d" % (self.env.now, self.bike_id, self.station_id, agent_id))
    ...
    def register_push(self, station_id):
        if self.print:
            print("[%.2f] Bike %d pushed on station %d by agent %d" % (self.env.now, self.bike_id, station_id, self.agent_id))
...
class DocklessBike(Bike):
    ...
    def unlock(self, agent_id):
        ...
        if self.print:
            print("[%.2f] Dockless bike %d unlocked by agent %d on location [%.2f, %.2f]" % (self.env.now, self.bike_id, agent_id, *self.location))

    def lock(self):
        if self.print:
            print("[%.2f] Dockless bike %d locked by agent %d on location [%.2f, %.2f]" % (self.env.now, self.bike_id, self.agent_id, *self.location))
...
class AutonomousBike(Bike):
    ...
    def call(self, agent_id):
        ...
        if self.print:
            print("[%.2f] Autonomous bike %d called by agent %d" % (self.env.now, self.bike_id, agent_id))

    def autonomous_move(self, location):
        if self.print:
            print("[%.2f] Autonomous bike %d moving from [%.2f, %.2f] to [%.2f, %.2f]" % (self.env.now, self.bike_id, *self.location, *location))

        distance = self.dist(self.location, location)
        yield self.env.timeout(distance)

        if self.print:
            print("[%.2f] Autonomous bike %d moved from [%.2f, %.2f] to [%.2f, %.2f]" % (self.env.now, self.bike_id, *self.location, *location))
    ...
    def drop(self):
        if self.print:
            print("[%.2f] Autonomous bike %d dropped by agent %d on location [%.2f, %.2f]" % (self.env.now, self.bike_id, self.agent_id, *self.location))
...
class Agent:
    ...
    def setup_task(self):
        ...
        if self.print:
            print("[%.2f] Agent %d task created" % (self.env.now, self.agent_id))
            print("[%.2f] Agent %d source location: (%.2f, %.2f) at %.2f" % (self.env.now, self.agent_id, *self.source, self.time_source))
            print("[%.2f] Agent %d target location: (%.2f, %.2f) at %.2f" % (self.env.now, self.agent_id, *self.target, self.time_target_desired))

    def init_agent(self):
        yield self.env.timeout(self.time_source)
        self.location = self.source
        if self.print:
            print("[%.2f] Agent %d initialized at location [%.2f, %.2f]" % (self.env.now, self.agent_id, *self.location))
    ...
    def walk_to(self, location):
        ...
        if self.print:
            print("[%.2f] Agent %d walked to location [%.2f, %.2f]" % (self.env.now, self.agent_id, *location))

    def ride_bike_to(self, location):
        if self.print:
            print("[%.2f] Agent %d heads from location [%.2f, %.2f] to location [%.2f, %.2f] on bike %d" % (self.env.now, self.agent_id, *self.location, *location, self.bike_id))

        bike = self.bikes[self.bike_id]
        yield self.env.process(bike.ride(location))

        if self.print:
            print("[%.2f] Agent %d reached from location [%.2f, %.2f] to location [%.2f, %.2f] on bike %d" % (self.env.now, self.agent_id, *self.location, *location, self.bike_id))
...
class StationAgent(Agent):
    ...
    def update_station_info(self, location):
        ...
        for station in self.stations:
            ...
            values.append((station_id, has_bikes, has_docks, visited, distance, walkable))
        labels = [
            "station_id",
            "has_bikes",
            "has_docks",
    ...
    def select_station(self, aim):
        ...
        for e in np.sort(self.station_info, order="distance"):
            ...
            if valid:
                ...
                if self.print:
                    print("[%.2f] Agent %d selected %s station %d" % (self.env.now, self.agent_id, aim, self.station_id))
                break
    ...
    def interact_bike(self, action):
        ...
        if valid:
            ...
            if self.print:  # AND HERE the action, the order is counfusing
                print("[%.2f] Agent %d %sed bike %d on station %d" % (self.env.now, self.agent_id, action, self.bike_id, self.station_id,))
            yield self.env.timeout(1)
        else:
            self.time_interact_bike = None
            if self.print:
                print("[%.2f] Station %d has zero %s available" % (self.env.now, self.station_id, "bikes" if action == "pull" else "docks",))
...
class DocklessAgent(Agent):
    ...
    def select_dockless_bike(self):
        ...
        for e in np.sort(self.bike_info, order="distance"):
            ...
            if valid:
                ...
                if self.print:
                    print("[%.2f] Agent %d selected dockless bike %d" % (self.env.now, self.agent_id, self.dockless_bike_id))
                break
    ...
    def unlock_bike(self):
        ...
        if not dockless_bike.rented():
            ...
            if self.print:
                print("[%.2f] Agent %d unlocked bike %d" % (self.env.now, self.agent_id, self.bike_id))
        else:
            yield self.env.timeout(3)
            self.time_unlock_bike = None
            if self.print:
                print("[%.2f] Bike %d has already been rented" % (self.env.now, self.dockless_bike_id))
    ...
    def lock_bike(self):
        ...
        if self.print:
            print("[%.2f] Agent %d locked bike %d" % (self.env.now, self.agent_id, self.bike_id))
...
class AutonomousAgent(Agent):
    ...
    def call_autonomous_bike(self):
        ...
        for e in np.sort(self.bike_info, order="distance"):
            ...
            if valid:
                ...
                if self.print:
                    print("[%.2f] Agent %d called autonomous bike %d" % (self.env.now, self.agent_id, self.bike_id))
                break
    ...
    def drop_bike(self):
        ...
        if self.print:
            print("[%.2f] Agent %d dropped bike %d" % (self.env.now, self.agent_id, self.bike_id))
