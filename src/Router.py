...
def get_haversine_distance(point_1, point_2):
    ...
    lon1, lat1, lon2, lat2 = map(math.radians, [point_1[0], point_1[1], point_2[0], point_2[1]])
    # haversine formula
    dlon = lon2 - lon1
    dlat = lat2 - lat1
...
class Network:
    ...
    def network_dfs_to_nx(self):
        ...
        for i, row in self.edges.iterrows():
            G.add_edge(row["from_node_id"], row["to_node_id"], weight=row["distance"])
        return G
    ...
    def get_path_distances(self, coord_path):
        distances = [get_haversine_distance(coord_path[i], coord_path[i + 1]) for i in range(len(coord_path) - 1)]
        return distances
